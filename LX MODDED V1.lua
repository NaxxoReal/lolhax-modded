...
ErrorMessageOut = game:GetService("LogService").MessageOut:Connect(function(Message, Type)
    ...
    if Type == Enum.MessageType.MessageError and not string.find(Message, "attempt to index nil with 'Value'") then
        ...
        Library:Notify(" Lolhax Modded has errored while loading and will now unload. The error has been copied to your clipboard! ", 4.5)
...
local Window = Library:CreateWindow({ Title = " LOLHAX MODDED V1 ┃ ".. LocalPlayer.Name, Center = true, AutoShow = true, TabPadding = 3, MenuFadeTime = 0.15 })
...
task.spawn(function()
    function updateRPC(Bool)
        ...
        print("[BloxstrapRPC] {\"command\": \"SetRichPresence\", \"data\": " .. game:GetService("HttpService"):JSONEncode({
            details = Bool and "[ Playing DOORS 👁️ ] Lolhax.modded" or "<reset>",
